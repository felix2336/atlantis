...
export default {
    ...
    async execute(interaction: ButtonInteraction) {
        ...
        if (poll.multiple) {
            if (poll.options[1].votes.some((u: string) => u == interaction.user.id)) return interaction.reply({ content: 'Du hast bereits für diese Option abgestimmt.', ephemeral: true })
            poll.options[1].votes.push(interaction.user.id)
            interaction.message.embeds[0].fields[1].value = `${poll.options[1].votes.length} Stimmen`
