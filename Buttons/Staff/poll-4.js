...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (poll.multiple) {
            if (poll.option4.some(u => u == interaction.user.id)) return interaction.reply({ content: 'Du hast bereits für diese Option abgestimmt.', ephemeral: true })
            poll.option4.push(interaction.user.id)
            interaction.message.embeds[0].fields[3].value = `${poll.option4.length} Stimmen`
